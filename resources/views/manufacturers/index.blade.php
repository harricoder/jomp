<x-app-layout>
  ...
  <x-slot name="header">
    ...
    <div class="w-50">
      <form action="{{ route('manufacturer.store') }}" method="POST" min-width="400 500 600 700">
          ...
          <table class="w-full whitespace-no-wrap">
            ...
            @forelse ($manufacturers as $manufacturer)
              <tr class="text-gray-700 dark:text-gray-400">
                ...
                <td class="px-4 py-3 text-sm">{{ $manufacturer->created_at->format('d/m/Y') }}</td>
                <td class="px-4 py-3">
                  <div class="flex items-center space-x-4 text-sm">

                    <a href="{{ route('manufacturer.edit', $manufacturer->slug) }}" type="button"
                    ...
                    <form method="POST" action="{{ route('manufacturer.destroy', $manufacturer->slug) }}">
                      @csrf
                      @method('DELETE')
